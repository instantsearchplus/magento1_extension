...
Mage::log('install 3.0.0.2 is triggered', null, 'autocomplete.log',true);
...
Mage::log('is config exists: ' . print_r($config, true), null, 'autocomplete.log',true);
...
try {
    $commandOrig = "http://0-2vk.acp-magento.appspot.com/install";
    ...
    Mage::log('sending install request', null, 'autocomplete.log',true);
    ...
    if ($errMsg != '') {
        ...
        $command = "http://0-2vk.acp-magento.appspot.com/install_error";
        $data = array();
        $data['site'] = $url;
        $data['msg'] = $errMsg;
        $data['email'] = $storeMail;
